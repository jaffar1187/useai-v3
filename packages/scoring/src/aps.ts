...
export function computeAPS(sessions: Session[]): APSResult {
  if (sessions.length === 0) {
    return { score: 0, components: { output: 0, efficiency: 0, promptQuality: 0, consistency: 0, breadth: 0 } };
  }
  ...
  const score = Math.round(
    Object.entries(WEIGHTS).reduce(
      (sum, [key, weight]) => sum + components[key as keyof APSComponents] * weight,
      0,
    ) * 1000,
  );
...
function computeOutput(sessions: Session[]): number {
  ...
  const avg = scored.reduce((sum, s) => sum + (s.score?.overall ?? 0), 0) / scored.length;
  return Math.min(1, avg);
}

function computeEfficiency(sessions: Session[]): number {
  const avgDuration = sessions.reduce((s, x) => s + x.durationMs, 0) / sessions.length;
...
function computePromptQuality(sessions: Session[]): number {
  ...
  return scored.reduce((sum, s) => sum + (s.score!.components["promptQuality"] ?? 0), 0) / scored.length;
}
